...
async def create_order(data: dict, user: dict) -> dict:
    ...
    # 如果前端沒傳入 amount，則使用單價（此處假設 amount 是單價 * 數量，若無數量則為單價）
    ...
    order_data = {
        ...
        "customer": data.get("customer", "Unknown"), # schemas.py 中有定義此欄位
    }
    ...
    if role == "sales":
        ...
        # admin 建立時可以使用傳入的 owner_id 或預設為自己
